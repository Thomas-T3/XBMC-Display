...
ISR(INT4_vect){
	...
	//Serial.print("/");
	//Serial.print(drehzahlsignal->overruns);
	//Serial.println("<-");
	drehzahlsignal->set_exact((uint32_t)60000000UL / (((uint32_t)drehzahlsignal->overruns<<16) + timerValue));
	drehzahlsignal->overruns=0;
}
...
ISR(TIMER1_OVF_vect){
	...
	drehzahlsignal->overruns++;
	if(drehzahlsignal->overruns>4){ // no spark for 164ms -> less than 183 rpm
		drehzahlsignal->set_exact(0);
		drehzahlsignal->overruns=0;
	}
}

void speedo_dz::set_exact(uint16_t i){
	//Serial.println("wenn diese zeile zusehen ist wird zumindest der interrupt getriggert!");
	//Serial.println(i);
	if(i>7000){
		exact=7000;
	} else if(i==0) {
